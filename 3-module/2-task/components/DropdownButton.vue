  ...
  <div class="dropdown show">
    ...
    <div class="dropdown__menu" :class="{ show: isActive }">
      <button
        ...
        :key="option.value"
        @click="handelItemClick(option)"
      >
        <app-icon :icon="option.icon" v-if="option.icon" />
        {{ option.text }}
      </button>
    </div>
  </div>
...
export default {
  ...
  computed: {
    customTitle() {
      let title;
      if (this.value) {
        let option = this.getOptionByValue(this.value, this.options);
        title = `${this.title} - ${option.text}`;
      } else {
        title = this.title;
      }
      return title;
    ...
    buttonIcon() {
      let option = this.getOptionByValue(this.value, this.options);
      return option ? option.icon : 'cal-sm';
    },
  },
  methods: {
    handleClick() {
